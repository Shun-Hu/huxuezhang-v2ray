...
showInfo() {
    ...
    if [[ "$vless" = "false" ]]; then
	    ...
	    echo -e "URL 二维码 (VLESS + TCP + XTLS) （请在浏览器中访问）"
	    echo -e "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://${uid}@${domain}:${port}?security=xtls&flow=$flow#huxuezhang-${IP}"
        elif [[ "$ws" = "false" ]]; then
            echo -e "   ${BLUE}IP(address):  ${PLAIN}${RED}${IP}${PLAIN}"
            echo -e "   ${BLUE}端口(port)：${PLAIN}${RED}${port}${PLAIN}"
            echo -e "   ${BLUE}id(uuid)：${PLAIN}${RED}${uid}${PLAIN}"
	    ...
	    echo -e "URL 二维码 (VLESS + WS + TLS) （请在浏览器中访问）"
	    echo -e "https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=vless://${uid}@${domain}:${port}?security=xtls&flow=$flow#huxuezhang-${IP}"
        else
            echo -e "   ${BLUE}IP(address): ${PLAIN} ${RED}${IP}${PLAIN}"
            echo -e "   ${BLUE}端口(port)：${PLAIN}${RED}${port}${PLAIN}"
